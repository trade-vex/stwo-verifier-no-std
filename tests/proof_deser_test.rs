...
use stwo_prover::constraint_framework::{FrameworkComponent, FrameworkEval, EvalAtRow, TraceLocationAllocator};
...
use stwo_prover::core::pcs::{CommitmentSchemeProver, PcsConfig as ProverPcsConfig, TreeVec};
...
// --- Verifier Imports ---
use stwo_verifier_no_std::types::proof::CommitmentSchemeProof;
use stwo_verifier_no_std::backend::{Blake2sHasher, Blake2sMerkleHasher};
use stwo_verifier_no_std::types::pcs::PcsConfig;
use stwo_verifier_no_std::types::fri::{FriProof, FriConfig};
    ...
    let n_packed_fields = 1 << (log_n_instances - LOG_N_LANES);
     let inputs = (0..n_packed_fields)
         .map(|i| TestFibInput {
             ...
             b: stwo_prover::core::backend::simd::m31::PackedBaseField::from_array(core::array::from_fn(|j| {
...
fn generate_prover_proof() -> ProverStarkProof<ProverBlake2sHasher> {
    ...
    let mut commitment_scheme =
        CommitmentSchemeProver::<SimdBackend, ProverMerkleChannel>::new(prover_config.clone(), &twiddles);
    ...
    let component = TestWideFibonacciComponent::new(
        ...
        TestWideFibonacciEval { log_n_rows: log_n_instances },
        ProverSecureField::zero(),
    );

    // Prove
    prove::<SimdBackend, ProverMerkleChannel>(
        &[&component],
        prover_channel,
        commitment_scheme,
    )
...
fn test_config_deser() {
    ...
    assert_eq!(proof.0.config.fri_config.log_blowup_factor, config_deser.fri_config.log_blowup_factor);
    assert_eq!(proof.0.config.fri_config.log_last_layer_degree_bound, config_deser.fri_config.log_last_layer_degree_bound);
    assert_eq!(proof.0.config.fri_config.n_queries, config_deser.fri_config.n_queries);
}
...
fn test_commitments_deser() {
    ...
    let commitments_deser: Vec<<Blake2sMerkleHasher as MerkleHasher>::Hash> = serde_json::from_str(&commitments_ser).unwrap();
...
fn test_sampled_values_deser() {
    ...
    let _sampled_values_deser: Vec<Vec<Vec<ProverSecureField>>> = serde_json::from_str(&sampled_values_ser).unwrap();
}
...
fn test_decommitments_deser() {
    ...
    let _decommitments_deser: Vec<MerkleDecommitment<Blake2sMerkleHasher>> = serde_json::from_str(&decommitments_ser).unwrap();
}
...
fn test_queried_values_deser() {
    ...
    let _queried_values_deser: Vec<Vec<ProverBaseField>> = serde_json::from_str(&queried_values_ser).unwrap();
}
...
fn test_fri_proof_deser() {
    ...
    let _fri_proof_deser: FriProof<Blake2sMerkleHasher> = serde_json::from_str(&fri_proof_ser).unwrap();
}
