...
mod tests {
    ...
    fn test_wide_fib_prove_with_blake() {
        for log_n_instances in 2..=6 {
            ...
            verify(&[&component], verifier_channel, commitment_scheme, proof.clone()).unwrap();
            
            let ser_proof = serde_json::to_string(&proof).unwrap();

            // Verify.
            use stwo_verifier_no_std::{verifier::verify as verify_no_std, types::StarkProof as StarkProofVerifier};
            use stwo_verifier_no_std::backend::Blake2sMerkleHasher as Blake2sMerkleHasherVerifier;
            use stwo_verifier_no_std::channel::Blake2sChannel as Blake2sChannelVerifier;
            use stwo_verifier_no_std::channel::Blake2sMerkleChannel as Blake2sMerkleChannelVerifier;
            use stwo_verifier_no_std::verifier::CommitmentSchemeVerifier as CommitmentSchemeVerifierVerifier;
            use stwo_verifier_no_std::types::PcsConfig as PcsConfigVerifier;

            let config2 = PcsConfigVerifier::default();
            let proof: StarkProofVerifier<Blake2sMerkleHasherVerifier> = serde_json::from_str(&ser_proof).unwrap();
            ...
            commitment_scheme.commit(proof.commitments[0], sizes_vec[0].as_slice(), verifier_channel);
            commitment_scheme.commit(proof.commitments[1], sizes_vec[1].as_slice(), verifier_channel);
