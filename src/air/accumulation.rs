...
use crate::backend::{Backend, Col, Column, ColumnOps, CpuBackend};
use crate::fields::m31::BaseField;
use crate::fields::qm31::SecureField;
use crate::fields::secure_column::SecureColumnByCoords;
use crate::poly::circle::{CanonicCoset, CircleEvaluation, CirclePoly, SecureCirclePoly};
...
impl<B: Backend> DomainEvaluationAccumulator<B> {
    ...
    //     let mut result: Vec<_> = n_cols_per_size
    //         .iter()
    //         .map(|(log_size, _)| {
    //             let index = *log_size as usize;
    //             self.sub_accumulations
    //                 .get_mut(index)
    //                 .unwrap_or_else(|| panic!("invalid log_size: {}", log_size))
    //         })
    //         .collect();

    //     // result: Vec<&mut Option<SecureColumnByCoords<_>>>
    //     result
    // }
