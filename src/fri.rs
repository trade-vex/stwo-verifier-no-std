...
use super::backend::{ColumnOps, CpuBackend};
...
use crate::vcs::prover::MerkleDecommitment;
use crate::vcs::verifier::{MerkleVerificationError, MerkleVerifier};
...
//     ) -> Self {
//         assert!(!columns.next().is_some(), "no columns");
...
pub fn fold_line(
    ...
    let mut iter = eval.values.into_iter();
    let folded_values = (0..eval.len() / 2)
        .map(|i| {
            let f_x = iter.next().unwrap();
            let f_neg_x = iter.next().unwrap();
...
pub fn fold_circle_into_line(
    ...
    for i in 0..dst.len() {
        let f_p = iter.next().unwrap();
        let f_neg_p = iter.next().unwrap();
